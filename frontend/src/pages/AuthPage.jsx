...
export default function AuthPage({ onAuth, dark, lang, setLang, onBack }) {
  ...
  const doGoogleLogin = () => {
    setErr(""); setLoadingKey("google");

    // If VITE_GOOGLE_CLIENT_ID is configured — use real Google One Tap
      ...
      window.google.accounts.id.initialize({
        ...
        callback: async ({ credential }) => {
          try {
            ...
            const { name: gName, email: gEmail, avatar: gAvatar } = res.prefill || {};
            setSocialEmail(gEmail || "");
            setName(gName || "");
            setAvatarB64(gAvatar || "");
            setMethod("google");
            setStep("profile");
          } catch (e) {
            setLoadingKey(null);
            setErr(e.data?.error || (lang === "bn" ? "Google লগইন ব্যর্থ" : "Google login failed"));
          }
        },
      });
