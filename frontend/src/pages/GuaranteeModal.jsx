...
export default function GuaranteeModal({booking,onClose}){
  ...
  const printGuarantee=()=>{
    ...
    <div class="header"><div class="logo"><Icon name="security" size={16} /></div>
    <div class="title">IMAP Service Guarantee</div>
    <div class="sub">সার্ভিস গ্যারান্টি সার্টিফিকেট</div>
    <div class="seal"><Icon name="success" size={14} style={{marginRight:6}} />VERIFIED & GUARANTEED</div></div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={onClose}>
      <div style={{
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <div style={{fontSize:16,fontWeight:700}}><Icon name="security" size={14} style={{marginRight:6}} />{lang==="en"?"Service Guarantee":"সার্ভিস গ্যারান্টি"}</div>
        ...
        <div style={{background:"linear-gradient(135deg,#D1FAE5,#A7F3D0)",borderRadius:14,padding:18,marginBottom:16,border:"2px solid #00C170",textAlign:"center"}}>
          <div style={{fontSize:36,marginBottom:8}}><Icon name="security" size={36} /></div>
        ...
        <div style={{display:"flex",gap:10,marginTop:16}}>
          <button onClick={printGuarantee} style={{flex:1,padding:"12px",borderRadius:12,background:"linear-gradient(135deg,#00C170,#004D38)",border:"none",color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"'Hind Siliguri',sans-serif"}}><Icon name="document" size={14} style={{marginRight:6}} />{lang==="en"?"Download PDF":"PDF ডাউনলোড"}</button>
          <button onClick={onClose} style={{padding:"12px 16px",borderRadius:12,background:C.bg,border:`1px solid ${C.bdr}`,color:C.text,fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"'Hind Siliguri',sans-serif"}}><Icon name="close" size={16} /></button>
        </div>
      </div>
    </div>
  );
