...
export default function ElderlyMode({onExit,onBook,onEmergency}) {
  ...
  const SERVICES=[{icon:"electrician",name:tr.elecProblem,p:pv[0]||PROVIDERS[0]},{icon:"utility",name:tr.waterProblem,p:pv[2]||PROVIDERS[2]},{icon:"healthcare",name:tr.docNurse,p:pv[1]||PROVIDERS[1]},{icon:"cleaning",name:tr.cleanService,p:pv[3]||PROVIDERS[3]}];
  return (
    <div style={{minHeight:"100vh",background:C.bg,padding:"24px 16px 88px",fontFamily:"'Hind Siliguri',sans-serif"}}>
      ...
      <div style={{
        ...
        <div style={{fontSize:46,marginBottom:8}}><Icon name="mic" size={46} /></div>
        ...
        <button className="jc" style={{width:68,height:68,borderRadius:"50%",background:`linear-gradient(135deg,${C.p},${C.pdk})`,border:"none",cursor:"pointer",fontSize:28,margin:"0 auto",boxShadow:`0 4px 20px rgba(16,24,20,0.11)`}}><Icon name="mic" size={16} /></button>
      </div>
    </div>
  );
}
